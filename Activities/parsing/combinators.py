...
# be used directly.)
#
# EXERCISE: Define Parser as a class (which must inherit fro Generic[A]).
...
def use(p: Parser[A], value: B) -> Parser[B]:
...
def optional(p: Parser[A], default: A | None = None) -> Parser[A | None]:
...
def repeated(p: Parser[A], minimum: int = 0, maximum: int = MANY_REPS) -> Parser[list[A]]:
...
def char_not_in(chars: str | list[str]) -> Parser[str]:
    ...
    def char_not(state: ParseState) -> ParseResult[str]:
        input, enough = state.require(1)
        if enough and input[0] not in chars:
            return Success(input, state.advance(1))
...
def string(s: str, transform: Callable[[str], str] = id) -> Parser[str]:
    ...
    def this_str(state: ParseState) -> ParseResult[str]:
        ...
        if enough and s == transform(input):
            return Success(input, state.advance(n))
...
def symbol(s: str, spacep: Parser[str] = space, transform: Callable[[str], str] = id) -> Parser[str]:
