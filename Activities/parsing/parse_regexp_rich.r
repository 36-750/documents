...
setClass("Intersection", slots = c(children = "list"),  contains = "RegExpBranch")
...
Intersection <- function(regexes) {
    if ( !is.list(regexes) ) regexes <- list(regexes)  # Handle singletons
    RegExpBranch("Intersection", children = regexes)
}
...
wrap_intersections <- function(res) {
    n <- length(res)
    if ( n == 1 ) {
        return( res[1] )
    }
...
regexp <- function(regexp) {
    ...
    intersects <- interleave(base(regexp), char('&')) %>>% wrap_intersections
