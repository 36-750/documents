...
from combinators import (Parser, ParseResult, Success, parse, failed,
                         ...
                         alt, alts, some, optional, interleave,
...
class Alternation(RegExpBranch):
    "Alternation of two or more regexes. (Alternation of a single regex should be the identity.)"
    def __init__(self, regexes: Iterable[RegExp]):
        self.children = list(regexes)

class Intersection(RegExpBranch):
    "Intersection of two or more regexes. (Intersection of a single regex should be the identity.)"
    def __init__(self, regexes: Iterable[RegExp]):
        self.children = list(regexes)
...
def wrap_intersections(res: list[RegExp]) -> RegExp:
    n = len(res)
    if n == 1:
        return res[0]
    elif n == 0:
...
def regexp(regexp):
    ...
    intersects = fmap( interleave(base(regexp), char('&')), wrap_intersections )
    term = alt(intersects, char('|'))
