...
** Grammars
   ...
   /production rules/ that specify how a single *nonterminal symbol* can be
   expanded into a series of other symbols (nonterminal, or *terminal* aka tokens).

   Most programming languages have *context free* grammars, meaning that
   the production rule for a nonterminal symbol applies regardless
   of the surrounding context. Such grammars can be expressed
   with single nonterminal symbols on the left-hand sides of production rules.
  
   Grammars are often written in (E)BNF -- (Extended) Backus-Naur Form.   

   Example: S-Expressions
   #+begin_example
     sexp -> atom
           | LPAREN sexp-list RPAREN

     sexp-list -> sexp sexp-list
                |

     atom -> NIL | BOOLEAN | NUMBER | STRING | IDENT | OPERATOR
   #+end_example
   Here, we've ignored whitespace completely with the idea
   that the input has been /tokenized/ first. Note the choice
   of ordering in =sexp-list=.

   Here are some valid strings in this language:
   #+begin_example
     foo
     4
     nil
     ...
     (())
     (foo 4)
     (bar (foo 4) (zap (wow 1 2 3)))
     (+ 4 2)
     (+ 1 (* 2 4) (/ 14 2))
   #+end_example
   Notice that this grammar corresponds strongly to a data type
   #+begin_src idris
     data Sexp = Atom
               | List Sexp
   #+end_src
   ...
   #+begin_example
     ...
     factor   -> SPACE* (grouping | let | NUM | var )
     ...
     SPACE    -> (" " | "\t" | "\n" | "\f" | "\r")
   #+end_example
   Here, we are representing space in the grammar, so we are handling
   spacing explcitly.

   *Ambiguity* occurs when there are more than one valid parse for a grammar.
   What is the ambiguity here? How do we resolve it?
...
** The Z-Combinator and Recursive Parsers

   sexp and regexp parsers in combinators library use an odd construction called =fix=.

   This uses the Z-combinator for generating recursive functions from non-recursive bases.

** Puzzlers

   1. Write IDENT parser
   2. Cochoice puzzle:  =cochoice : (Either a c -> Either c b) -> a -> b=
   3. =Free f a= and =Cofree f a=

** Future Work

   Work through at least the Natural Number Game

   + [[https://adam.math.hhu.de/#/g/leanprover-community/nng4][Natural Number Game]]
   + [[https://adam.math.hhu.de/#/g/djvelleman/stg4][Set Theory Game]]
   + [[https://adam.math.hhu.de/#/][Lean Game Server]]
   + [[https://www.uv.es/coslloen/Arxiu/Fitxers/An-introduction-to-Lean-4.pdf][An Introduction to Lean 4]]
...
** Starters: Machine Representations of Data

*** Integers

    + Unsigned integers :: n binary bits as /unsigned/ numbers can represent
                           from 0 to $2^n - 1$

      Typical sizes 8 bit, 16 bit, 32 bit, 64 bit (standard integer)

    + Signed integers :: sign-magnitude style

      Reserve msb for sign indicator, remaining bits for absolute value.

    + Signed integers :: ones-complement style

      Again msb for sign bit, remaining bits for the absolute value, but
      when the sign bit is set, we store the *complement* of the absolute value.

      (Absolute value of entire number is the complement of the number!)

    + Signed integers :: twos-complement style

      This is the standard style used on most machines.

      Bit i in 0 .. n-2 represents 2^i. The msb represents -2^{n-1}.
      This acts as a sign bit but it does multiple things:
      i. there is a unique representation of 0, and ii. ordinary
      arithmetic operations just work (at the circuit level).

*** Floating Point Numbers

    [[https://en.wikipedia.org/wiki/IEEE_754][IEEE 754]] is a technical standard for representation of and operations on machine floating-point
    numbers. It provides a portable convention that is used by most common hardware today.

    A floating point number (say 64) bits is divided into
   
    + a sign bit S (1 bit)
    + an exponent E (11 bits)
    + a fraction F (52 bits)

    There are three key forms:

    1. Normalized form (1 <= E <= 2046)

       (-1)^S * 1.F * 2^{E - 1023}

       Note that F here has an /implicit/ leading 1 bit. It is not stored.

    2. Denormalized form (E = 0)

       (-1)^S * 0.F * 2^{-1022}

       Key issue here is that we get a representation of 0.

    3. Special form (E = 2047)

       The remaining bits encode special numbers including +/- =Inf=
       (infinities) and =NaN= (Not a Number).

    Typically =double= precision is 64 bits and single precision is often 32 bits (which
    has different numbers of bits in the E and F segments).

*** Boxing, Objects, and Pointers

    *Pointers* are values (typically 64 bits) that represent an /address/ in memory.
    With a pointer, you can *derefeference* that address to get the value at
    the specified address.  This is a form of memory indirection that
    allows to refer to complicated data structures stored elsewhere in memory.
    (This is often depicted pictorially.)

    In high-level languages like Python or R, a wide variety of data is treated
    as part of the objects system, with bells and whistles like method dispatch
    and runtime introspection. Data that has been wrapped in an object
    (formal or informal) is often referred to as *boxed*. We think of it
    as data wrapped in a context in which we can refer to it and use
    it in a higher-level framework. The opposite -- *unboxed* data -- is
    stored directly in its machine representation.

    Examples: Python ints, arrays, objects

*** Little Endian vs. Big Endian

    Storing multi-byte number in memory. Little endian puts the least significant byte
    at the lowest memory address. Big endian puts the least significant byte at the
    biggest memory address.
...
** Caches
   ...
   If you need data that is not in the cache -- called a *cache miss*
   -- the processor needs to get the data from memory, which is
   effectively a penalty on the order of 500 clock cycles.

   If data can't be found in the L1 cache, the processor will look
   in the L2, then the L3 and so on down the line. The (typically)
   increasing sizes of the higher caches increases the hit rate
   and gives a performance improvement over a broader range of code.
   Some processors include a copy of L1 in L2 etc (inclusive);
   others prevent data from being shared among caches (exclusive).

   There are also subtleties with the association of blocks of main
   memory with blocks of cache memory. This property is called *set
   associativity*. At one extreme, any block of main memory can go
   into any block of cache. This increases the hit rate but also
   search times. At the other extreme, each block of memory can be
   associated with a unique block of cache. Low hit rate, easy to
   find.

   The processor automatically manages the cache. You can't directly
   manipulate it with your code. The cache contains copies of
   frequently used chunks of memory, automatically discarding the
   least recently used chunks to make space for new ones. When your
   program uses a certain memory location, an entire chunk of memory
   containing that location is copied into the cache.
   ...
   + Suppose the CPU has to load data from L1 100 times in a row.
     If the L1 latency is 1ns, the time for this operation becomes 100ns
     because the hit rate is 100%.

   + Suppose that the data needed is in L1 for the 99 attempts and L2
     for the last. Assume an L2 latency of 10ns. Then this operation
     takes 109ns. A 1% drop in hit rate gives a 9% change in performance.

   + If the last attempt is in main memory (latency say 100ns), we
     get a 199ns operation, almost a 100% slowdown.

   + In practice, if the hit rate is about 95% for L1, you can see
     the impact of even that 5%.
...
** Cache overview
   ...
   + Pay attention to indirect memory references
   ...
   boxed values are stored as /references/, requiring an
   extra non-local access to get their value. (Now consider
   a hash table storing boxed values, indirections on
   top of indirections.)
...
** Data Representations
  - [[https://www.sciencedirect.com/topics/computer-science/twos-complement-number][Two's Complement Numbers]]
  - [[https://www3.ntu.edu.sg/home/ehchua/programming/java/datarepresentation.html][A Tutorial on Data Representations]]
...
** Parsing
  - [[https://www.cs.cornell.edu/courses/cs2112/2021fa/lectures/parsing/][Grammars and Parsing]]

** The Z-Combinator
  - https://thenewobjective.com/types-and-programming-languages/deriving-the-z-combinator/
  - https://thenewobjective.com/types-and-programming-languages/implementing-classes-with-the-z-combinator/
