...
class RegExp:
    ...
    # EXERCISE: Implement a pretty printer
...
class Dot(RegExpLeaf):
    """The dot . metacharacter, matches any non-newline character."""
...
class Group(RegExpBranch):
    ...
    def pretty(self, level=0, indent=4) -> str:
        ...
        indentation = ' ' * (level * indent)
        name = self.__class__.__name__
        group_name = f', name={self.name}' if hasattr(self, 'name') else ''
        return f'{indentation}{name}[type={self.type}{group_name}]\n' + self.child.pretty(level + 1)
...
class Repetition(RegExpBranch):
    ...
    def pretty(self, level=0, indent=4) -> str:
        ...
        indentation = ' ' * (level * indent)
        name = self.__class__.__name__
        return f'{indentation}{name}[type={self.type}, min={self.minimum}, max={self.maximum}, lazy={self.lazy}]\n' +\
               self.child.pretty(level + 1)
