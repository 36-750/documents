#
# A simple Lens implementation
# 
# alias Applicative f => Lens a b s t = (a -> f b) -> s -> f t
#
...
from functions   import compose, pair, triple

__all__ = [
    'lens', 'view', 'collect', 'over', 'set_',   # name set_ avoid conflict with builtin set
    'mapped',
...
def seq(lens1, lens2):
    "Activate two lenses in sequence, assuming the types match and f is a monad."
    def in_seq(a_fb):
        ell1 = lens1(a_fb)
        ell2 = lens2(a_fb)
        def activate(s):
...
if __name__ == '__main__':
    ...
    view(chars(2), First)(cc)
    #=> 'c'
    
    set_(chars(9), "!")(cc)
    #=> 'abcdefghi!klmnopqrstuvwxyz'
    
    over(chars(9), lambda c: c.upper())(cc)
